 ...
 *
 * 走りだしの状態は見ない。#18-8-C で 4 つの実体はこの module の static に
 * なったが、ゲームの開始時に main.c が覚えた順を SPELL_NONE で埋め
 * （spell_order_forget_all）、続きから始めるときはセーブファイルが 4 つとも
 * 上書きするので、**初期値そのものは誰も読まない**。下のどのテストも
 * まっさらから始めるので、ここから観測もできない。
 */
