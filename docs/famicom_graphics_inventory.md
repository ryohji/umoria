...
- **最小**: 壁 1 枚（＋鉱脈 2 枚）。
- **推奨（試作で採用）**: 壁は床に面した辺にだけ岩（通路）やレンガ
  （部屋）を描き、奥は黒にして未探索の闇に溶けこませる。どの辺が床に
  面するかの 16 通りは、8×8 のチップを 1/4 ごとに選んで組む
  （1/4 に効くのはその角をはさむ 2 辺だけ）。岩の壁は 21 枚、レンガの
  壁は 10 枚のチップで全部そろう。岩はパレット（属性）の振りかたで同じ
  チップを 3 通りの色に描き分ける（`docs/famicom_dungeon_tileset.md`）。
- 共通の背景色（パレットの 0 番）を**床の色**にしておくのが前提。
  そうすると壁のチップも床の色を使えるので、1 マス 1 パレットのまま
  岩と床の境目をぎざぎざにできる。
