...
- **最小**: 壁 1 枚（＋鉱脈 2 枚）。
- **推奨**: 「下が床の壁」だけ正面の面を描く 2 枚組（上面・正面）。
  ファミコンの見下ろし RPG の定番で、奥行きが出る。
- **凝る場合**: 上下左右の隣が壁かどうかで 16 通りのオートタイル。
  3 種の壁 × 16 = 48 枚になるので、まずは推奨案で足りる。
...
| `D` | 古代竜 | 6 | 38–40 | 1 | 同上。`d` より大きく見える絵に |
| `B` | バルログ | 1 | 100 | 1 | **最後の敵**。32×32 の大きな絵（16×16 を 4 マス）にする価値がある |
...
### 8.2 提案: 「つながり」を描く罫線マップ

**1 タイル（8×8）＝ ダンジョンの 7×6 マスの区画**とする。外周を除いた
196×64 マスは **28×11 タイル**（224×88 ドット）に収まる。縦をもっと
使うなら **7×3 マス → 28×22 タイル**（原典の縦横比に近い）。

区画ごとに次を決め、タイルを選ぶ。

1. **種類**: 覚えている床のうち部屋の床（`fval <= MAX_CAVE_ROOM`）が
   あれば「部屋」、通路の床（`CORR_FLOOR` `BLOCKED_FLOOR`）だけなら
   「通路」、どちらも無ければ「空白」。
2. **つながり（上・右・下・左の 4 ビット）**: 区画の境目をはさんで、
   覚えている通れるマスが隣の区画のマスと接していればその方向の
   ビットを立てる。

| タイル群 | 数 | 絵 |
|---|---:|---|
| 空白（未探索） | 1 | 黒 |
| 通路（一重線） | 16 | 4 ビットの全組み合わせ: ─ │ ┌ ┐ └ ┘ ├ ┤ ┬ ┴ ┼、行き止まりの 4 つ（╴ ╵ ╶ ╷）、孤立した点 ・ |
| 部屋（二重線の枠＋中を網かけ） | 16 | 隣も部屋の方向は枠を描かず、部屋でない方向だけ ═ ║ を描く 4 ビット。1 区画だけの部屋は □、大きい部屋は ╔═╗ … ╚═╝ の組になる |
| 目印 | 4〜6 | 上り階段、下り階段、（町では）店 1〜6 は数字タイルで代用 |
| 枠 | 第 7 節と共用 | ╔ ╗ ╚ ╝ ═ ║ |

**合計およそ 37 枚**。一重線と二重線は第 7 節の窓枠と同じものなので、
重ねればさらに減る。

- **プレイヤー**は BG ではなくスプライトで点滅させる（区画の中の位置に
  1〜2 ドット単位で置けるので、7×6 より細かい位置がわかる）。
- 見えているモンスターを出すなら、同じくスプライトの小さな点で。
  ファミコンのスプライトは横 1 列に 8 個までなので、多いときは
  点滅で回す。
- 原典の優先度（`@` と階段がいちばん強い）を守り、**階段は区画の中に
  あれば必ず目印タイルを出す**。

計算量は 12,000 マス余りを 1 回なめるだけなので、マップを開いたときに
数フレームで作れる。区画ごとの 4 ビットを「マスを覚えた瞬間」に
更新しておけば、開くのは一瞬になる（区画数 28×11 = 308 バイト）。

### 8.3 もっと安い代案: 2×2 ブロック

1 タイルを 4×4 ドットの 4 升に割り、升ごとに「通れる所を覚えているか」を
塗る。絵は **16 枚**（▘▝▖▗ の全組み合わせ）だけ。1 升 ＝ 3.5×3 マスに
すれば 56×22 升 ＝ 28×11 タイル。線ではなく面の地図になり、細い通路が
途切れて見えやすいので、罫線案のほうが読みやすい。
...
本題から外れるが、原典のダンジョン配列（198×66 マス）は本体 RAM 2KB に
入らないので、どの方式でもカートリッジ側に 8KB 以上の RAM が要る。
