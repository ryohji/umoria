...
| 18 | データの散在 | `externs.h` の 98 個（着手時 112。misc3.c:369-540, dungeon.c:580-780 ほか） | プレイヤー状態値が py.misc / PY_* ビット / 画面座標 に分散。**2026-09-09 に全体像を計測 → [GLOBALS_INVENTORY.md](GLOBALS_INVENTORY.md)** | 区分ごとに分割して着手（下記）。オプション 11 個・メッセージ 4 個・パネル 10 個は完了 | High | 区分ごと |
...
- **着手順は「安い順」。** オプション 11 個（`misc2.c:856` に既に登録表が
  ...
  **オプションとメッセージは 2026-09-11、パネルは 2026-09-13 に完了
  （下の #18-1 / #18-2 / #18-3）。次は店 2 個・持ち物 6 個。どちらも
  別名（`store` 23・`inventory` 105）が多く、ここから先は「窓口を作って
  参照を置きかえる」だけでは済まない。**
