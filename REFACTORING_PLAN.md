...
`make` の警告は **8,338 件 → 2 件**。残る 2 件はふるまいが変わる修正が必要なもの
（下の「持ちこし」）。テストは 285 件 → 301 件でグリーン。
...
### 持ちこし（ふるまいが変わるので別に判断する）

| 場所 | 内容 | 決めること |
|---|---|---|
| `save.c:768-769` | `rd_short((uint16_t *)&panic_save)` が 1 バイトの `bool` に 2 バイト読みこむ。書きだし側（`save.c:247-248`）も `wr_short` なのでファイル形式は本当に 2 バイト。直すには局所の `uint16_t` に読んでから `panic_save = (tmp != 0)` とする | セーブファイル読みこみ時のふるまいが変わる |
| `monsters.c:709` | `monster_creature_rend()` が `c_list - 1`（配列の直前）を返す。終端の 1 つ先は合法だが先頭の 1 つ前は未定義動作。呼びだしは 2 箇所（`help.c:299`, `main.c:260`） | `rbegin`/`rend`/`prev` の三つ組の設計を変えることになる |
